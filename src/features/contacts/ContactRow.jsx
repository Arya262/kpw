...
export default function ContactRow({
  ...
  return (
    ...
    >
      ...
      <td className="px-2 py-4 sm:px-6 sm:py-4 text-[12px] sm:text-[16px] text-green-700 font-medium">
        {contact.status}
      </td>
      <td className="px-2 py-4 sm:px-6 sm:py-4 text-[12px] sm:text-[16px]">
        <Tooltip text={contact.fullName} position="top">
          <div
            onClick={handleChat}
            className="text-gray-700 cursor-pointer hover:text-[#0AA89E] font-medium truncate"
          >
            {contact.fullName}
          </div>
        </Tooltip>
        ...
        <div className="flex justify-center">
          <span
            className={`inline-block px-3 py-1 rounded-full text-white text-sm min-w-[80px] text-center font-medium
              ${contact.is_active === 1 ? "bg-green-600" : "bg-red-600"}`}
          >
