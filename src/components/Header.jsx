...
export default function Header({ isMenuOpen, onToggleSidebar }) {
  ...
  const handleOnboard = async (name) => {
    ...
    try {
      ...
      notify("error", err.response?.data?.error || "Something went wrong during onboarding.");
    } finally {
...
>
  ...
  <div className="text-sm text-gray-500 italic font-medium whitespace-nowrap hidden sm:block">
    WhatsApp Number:
  </div>
