...
/>
          ...
          {(user?.status === "inactive" || user?.status === "suspend") && (
            <button
              onClick={() => {
                if (user.status === "inactive") {
                  // Generate brand name dynamically from user info
                  const brandName = user?.name
                    ? user.name.toLowerCase().replace(/\s+/g, "")
                    : `user${user?.customer_id}`;
                  handleOnboard(brandName);
                } else if (user.status === "suspend") {
                  // Show modal for suspended users
...
{location?.loaded && !location?.error && location?.address && (
  <div className="mt-2 text-xs text-gray-500 flex items-center justify-center gap-1">
    ...
    <span
      ...
      title={typeof location.address === "string" ? location.address : location.address?.state}
    >
